...
{
	constexpr size_t writeUtf8(char* data, unsigned int code)
	{
		// U+0000..U+007F
		if (code < 0x80)
		{
	...
	constexpr std::optional<double> toDouble(std::string_view string)
	{
		double result;
	...
	constexpr std::optional<double> toDouble(const char* string) {
		return toDouble(std::string_view(string));
	}

	constexpr std::optional<double> toDouble(const std::string& string) {
		return toDouble(std::string_view(string));
	}

	constexpr std::string toString(double value)
	{
		char buffer[32];
	...
	constexpr std::string pointerToString(void* pointer)
	{
		char buffer[20];
	...
	constexpr std::string pointerToString(uintptr_t pointer) {
		return pointerToString((void*)pointer);
	}

	constexpr uintptr_t stringToPointer(const char* str) {
		return std::strtoull(str, nullptr, 16);
	}

	constexpr uintptr_t stringToPointer(const std::string& str) {
